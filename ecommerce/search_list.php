...
$stmt = $pdo->prepare('SELECT * FROM items WHERE name LIKE :word');
$stmt->execute(array( ':word' => "%".$_REQUEST['term']."%"));
$items = array();
    ...
    <main>
        ...
        function addFav(itemid, userid) {
              ...
              $.ajax({
                  ...
                  success: function () {
                      $("#button" + item_id)
                      .replaceWith('<button id="button'+item_id+ ' onclick="delFav('+item_id+','+user_id+')" style="background-color:white; border: none;">\
                      <i class="fa fa-heart" style="color:red" id="favourites'+item_id+'">\
                      </button>');
                    },
              });
          }

          function delFav(itemid, userid) {
              var user_id = userid;
              var item_id = itemid;

              $.ajax({
                  type: 'post',
                  url: '/estore/unlike.php',
                  data: {
                      'user_id': user_id,
                      'item_id': item_id,
                  },
                  success: function () {
                      $('#button' + item_id)
                      .replaceWith('<button id="button'+item_id+ 'onclick="addFav('+item_id+','+user_id+') style="background-color:white; border: none;"">\
                      <i class="far fa-heart" style="color:red" id="favourites'+item_id+'">\
                      </button>');
                  }
              });
          }
                ...
                <div class="row product-btn d-flex justify-content-end align-items-end">
                    ...
                    <div class="col-xl-4 col-lg-5 col-md-5">
                        <div class="section-tittle mb-30">
                            <h2>Products related to <?php echo $_REQUEST['term'];?> </h2>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
                        <div class="row">
                          ...
                          for($i=0; $i<sizeof($items); $i++)
                          {
                            ...
                            $stmt->execute(array(':id' => $items[$i]['img_id']));
                            ...
                            echo("<div class=\"col-xl-4 col-lg-4 col-md-6\">
                                <div class=\"single-product mb-60\">
                                    ...
                                    <div class=\"product-caption\">
                                        ...
                                        if(isset($_SESSION['user_id'])){
                                          $stmt = $pdo->prepare('SELECT * FROM wishlist WHERE user_id = :uid AND item_id=:iid');
                                          $stmt->execute(array( ':uid' => $_SESSION['user_id'], ':iid'=> $items[$i]['item_id']));
                                          $row = $stmt->fetch(PDO::FETCH_ASSOC);
                                            if ($row==false) {
                                              echo("<button id=\"button".$items[$i]['item_id']."\" onclick=\"addFav(".$items[$i]['item_id'].",".$_SESSION['user_id'].")\"
                                              style=\"background-color:white; border: none;\">
                                              <i class=\"far fa-heart\" style=\"color:red\" id=\"favourites".$items[$i]['item_id']."\">
                                              </i></button>");
                                            }
                                            else {
                                              echo("<button id=\"button".$items[$i]['item_id']."\" onclick=\"delFav(".$items[$i]['item_id'].",".$_SESSION['user_id'].")\"
                                              style=\"background-color:white; border: none;\">
                                              <i class=\"fa fa-heart\" style=\"color:red\" id=\"favourites".$items[$i]['item_id']."\">
                                              </i></button>");
                                            }
                                      }
                                      echo("</div>
                                        <h4><a href=\"single-product.php?itemid=".$items[$i]['item_id']."\">".$items[$i]['name']."</a></h4>
                                        <div class=\"price\">
                                            <ul>
                                                <li>$".$items[$i]['price']."</li>
                                                <li class=\"discount\">$".$items[$i]['price']*1.25."</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
