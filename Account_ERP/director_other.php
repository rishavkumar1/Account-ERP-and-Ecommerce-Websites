...
if (isset($_POST['done']) && isset($_SESSION['username'])) {
  ...
  $dest_other = 'director_registration_files/other_files/' . $filename_other;
  $filename_noc = $_FILES['noc']['name'];
  $file_noc = $_FILES['noc']['tmp_name'];
  $dest_noc = 'director_registration_files/noc/' . $filename_noc;
  $emailID = $_SESSION['username'];
  move_uploaded_file($file_other, $dest_other);
  move_uploaded_file($file_noc, $dest_noc);
  $upd= "UPDATE users SET Director_other_file = '$filename_other', Director_noc = '$filename_noc'  WHERE email ='$emailID'";
  if (mysqli_query($conn, $upd)) {
    header("Location: director_finish.php");
  }
  else{
...
<html>
<head>
  <title> Director Other Document Upload Page </title>
  <link rel="stylesheet" type="text/css" href="format.css" media="screen"/>
</head>
<body>
<?php echo $message; ?>
<form method='post'  action="director_other.php" enctype="multipart/form-data">
  <p> <label for="01"> Upload Other Document</label>
  <input type="file" id="01" name="other" required></p>
  <a href="director_registration_files/director_noc.docx">Download NOC</a>
